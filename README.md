...
화면은 IDE형 4열이다 — 좌측 액티비티 바가 **사이드바 내용**을 바꾸고(파일 트리 · 규칙 · 스킬 · 훅 · MCP), 중앙 에디터는 언제나 열린 파일의 탭·줄번호 원문(편집 모드 포함)이며, 우측 인스펙터가 선택한 것의 세부(스캔 정보·속성·항목 설명·토글·선택한 훅/MCP·함께 적용되는 규칙·프로젝트 훅·재스캔/종료)를 맡는다. 어느 사이드바에서든 항목을 클릭하면 에디터에 탭으로 열리고 목록은 그대로 남는다. 다크 기본, OS 라이트 테마를 따른다.
...
- **섹션** — CLAUDE.md 를 마크다운 헤딩(`#`~`######`)으로 잘라 다룬다. 펜스 코드블록(``` / ~~~) 안의 `#` 는 헤딩으로 보지 않고, 첫 헤딩 앞 내용은 전문(preamble)으로 둔다. setext 헤딩(`===`·`---` 밑줄)은 인식하지 않는다.
  ...
  - **접기** — 원문 뷰의 헤딩 줄 번호 칸 토글로 섹션을 접고 편다. 툴바의 "모두 접기 / 모두 펼치기"도 있다. 접힘 상태는 저장·재읽기·재스캔 후에도 유지된다.
  ...
  - **충돌(V5)** — 규칙 사이드바의 섹션 행에 "충돌" 배지가 붙고, 인스펙터의 충돌 섹션에서 파일별 본문을 나란히 펼쳐 볼 수 있다. 헤더를 누르면 그 파일의 해당 줄로 이동한다.
  - **프로젝트 간 비교** — 규칙 사이드바 상단 검색창에 제목을 넣고 Enter(또는 섹션 행의 ⇄ 버튼)를 누르면, 제목이 **완전히 일치**하는 섹션을 전역·모든 프로젝트에서 모아 비교 탭으로 보여준다. 비교 탭은 읽기 전용이다.

- **토글** — 스킬 사이드바의 행 오른쪽 스위치로 이 프로젝트의 스킬·커맨드를 ON/OFF 하고, 항목을 열면 인스펙터에서 4단계(`on` · `name-only` · `user-invocable-only` · `off`)로 지정한다. 플러그인은 사이드바 섹션 헤더에서 **플러그인 단위로만** 켜고 끈다(플러그인 내부 스킬 개별 토글은 없다). 에이전트는 토글 대상이 아니다.

  - 토글하려면 파일 사이드바에서 프로젝트를 먼저 선택해야 한다. 전역 설정 토글은 지원하지 않는다.
  - 저장 대상은 기본이 프로젝트의 `.claude/settings.local.json`(개인)이고, 인스펙터의 토글 섹션에서 `.claude/settings.json`(팀 공유)으로 바꿀 수 있다. 팀 공유를 고르면 "커밋하면 팀 전체에 적용됨" 경고가 뜬다.
